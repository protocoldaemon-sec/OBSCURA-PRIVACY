...
export class WOTSKeyManager {
  private pool: WOTSKeyPool;
  private scheme: WOTSScheme;
  private merkleTree: MerkleTree;

  private constructor(pool: WOTSKeyPool, scheme: WOTSScheme, merkleTree: MerkleTree) {
    this.pool = pool;
    this.scheme = scheme;
   ...
   */
  static async create(options: KeyPoolOptions): Promise<WOTSKeyManager> {
    const {
      keyCount,
      w = 16,
      seed,
  ...
  verifySignedIntent(signedIntent: WOTSSignedIntent, expectedRoot?: Hash): boolean {
    try {
      ...
      const isValidSig = this.scheme.verifyWithPublicKey(
  ...
  private getNextUnusedKey(): WOTSKeyEntry | null {
  ...
  private burnKey(index: number, intentHash: Hash): void {
